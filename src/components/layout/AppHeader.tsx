...
const navLinkClass = ({ isActive }: { isActive: boolean }) =>
  cn(
    ...
    isActive
      ? "rounded-none bg-neon text-white shadow-[0_12px_30px_rgba(29,50,108,0.2)]"
      : "rounded-none bg-white text-ink hover:bg-[#eef3ff] hover:text-neon"
  );

const actionClass =
  "inline-flex min-h-[3rem] w-full max-w-full shrink-0 items-center justify-center gap-2 rounded-none bg-white px-6 text-sm font-semibold text-ink transition duration-200 hover:bg-[#eef3ff] hover:text-neon sm:w-auto sm:whitespace-nowrap";
...
export function AppHeader() {
  ...
  const accountLinks = (
    <>
      {isSignedIn && (
        <span className="inline-flex min-h-[3rem] w-full shrink-0 items-center justify-center rounded-none bg-[#eef3ff] px-4 text-sm font-semibold text-neon sm:w-auto sm:px-6 sm:whitespace-nowrap">
          Role: {role}
        </span>
      )}
      ...
      {isSignedIn ? (
        <motion.button type="button" onClick={handleSignOut} {...buttonHover} className={actionClass}>
  ...
  return (
    <header className="bg-white px-4 py-4 md:px-6">
      <nav className="mx-auto flex w-full max-w-[112rem] flex-col items-center gap-4 rounded-none border border-ink/10 bg-white px-4 py-4 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:rounded-none sm:px-5 lg:flex-row lg:flex-nowrap lg:justify-between lg:px-8">
        <div className="flex w-full items-center justify-between gap-4 lg:w-auto">
          ...
          <motion.button
            ...
            {...buttonHover}
            className="inline-flex h-12 w-12 items-center justify-center rounded-none bg-[#eef3ff] text-neon lg:hidden"
